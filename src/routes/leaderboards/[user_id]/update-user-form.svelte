<script lang="ts">
	...
	import type { RequestStatus, Result, Section, User } from '$lib/types';
	...
	export let sections: Section[] | Result;
	export let currentUserData: User | Result;
...
{#if isResult(currentUserData)}
	<Alert.Root variant="destructive">
		<AlertCircle class="h-4 w-4" />
		<Alert.Title>Error</Alert.Title>
		<Alert.Description>
			<p>Failed to fetch user.</p>
			<p>{currentUserData.message}</p>
		</Alert.Description>
	</Alert.Root>
{:else if isResult(sections)}
	<Alert.Root variant="destructive">
		<AlertCircle class="h-4 w-4" />
		<Alert.Title>Error</Alert.Title>
		<Alert.Description>
			<p>Failed to fetch sections.</p>
			<p>{sections.message}</p>
		</Alert.Description>
	</Alert.Root>
{:else}
			...
			<Form.Field {config} name="role">
				<Form.Item>
					...
					>
						...
						<Form.SelectContent>
							<Form.SelectItem value="user" class="text-base md:text-lg h-auto"
								>User</Form.SelectItem
							>
