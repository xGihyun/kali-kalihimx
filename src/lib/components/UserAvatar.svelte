<script lang="ts">
	import type { Dimensions, RequestStatus, Result, User } from '$lib/types';
	...
	let isPrivate = isResult(user) ? false : user.is_private;
	const initials = isResult(user) ? '--' : (user.first_name[0] + user.last_name[0]).toUpperCase();
