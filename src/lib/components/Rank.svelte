<script lang="ts">
	import { RANK_LOGO_COLORS, getRankTitle } from '$lib';
...
<Card.Root>
	<Card.Header>
		<Card.Title class="text-4xl font-normal font-jost-bold">Performance</Card.Title>
	</Card.Header>
	<Card.Content class="flex px-0 relative flex-wrap">
		<div class="flex flex-1 items-center gap-6 relative px-6 pl-9 pr-6 h-32">
			<!-- <div -->
			<!-- 	class={` absolute left-0 top-0 w-full h-full ${RANK_LOGO_COLORS.get( -->
			<!-- 		getRankTitle(user.score) || '' -->
			<!-- 	)}`} -->
			<!-- ></div> -->
			<div
			...
			<span class="font-jost-bold text-4xl uppercase flex-1 relative z-20"
		...
		<div class="flex-1 flex gap-8 items-center px-6 py-6">
			<div class="flex flex-col">
				...
				<span class="text-2xl font-jost-medium">{user.score}</span>
			</div>

			<div class="flex flex-col">
				<h4 class="text-muted-foreground text-sm md:text-base">Overall</h4>
				<span class="text-2xl font-jost-medium">#{user.rank_overall}</span>
			</div>

			<div class="flex flex-col">
				<h4 class="text-muted-foreground text-sm md:text-base">Section</h4>
				<span class="text-2xl font-jost-medium">#{user.rank_section}</span>
			</div>
		</div>
	</Card.Content>
</Card.Root>
